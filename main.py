...
# SLICER
# CURA
from slicer.cura import slicing as slicu
...
from rhino.process import import_robot as rhbot
...
try:
    settings = sulod.load_settings(setup_path)
    suval.validate_settings(settings)
...
OUTPUT_SPLIT_SRC = directory_settings["split_src"]["value"]
...
# KRL
OUTPUT_DIRECTORY_KRL = output_folder
...
ROBOT_START_CODE_JSON = [
    surpl.replace_placeholders(line, settings, precision)
...
CURA_PRINTER_CONFIG_FILE_PATH = cura_settings["cura_printer_config_file_path"]["value"]
...
if SLICER == "cura":
    ...
    scale = cusca.compute_scaling_and_rotation_matrix(CURA_SCALING)
    ...
    default_data_printer = cuext.load_json(
        str(default_data_path / "TUM_C3DP_fdmprinter.def.json")
    )
    default_data_extruder = cuext.load_json(
        str(default_data_path / "TUM_C3DP_fdmextruder.def.json")
    )
    ...
    printer_default_values = cuext.extract_settings(
        default_data_printer.get("settings", {})
    )
    extruder_default_values = cuext.extract_settings(
        default_data_extruder.get("settings", {})
    )
...
gcode_lines = getgc.get_gcode_lines(OUTPUT_DIRECTORY_GCODE, OUTPUT_FILE_GCODE)
...
print(f"[INFO] Checking start position {ROBOT_START_POSITION} of robot from setup.json")
...
linewidths, volume = puliw.get_linewidth(
    gcode_necessary, PUMP_LINETYPE_FLOW, PUMP_FILAMENT_DIAMETER
)
for gcd, lw in zip(gcode_necessary, linewidths):
    gcd["Linewidth"] = lw

# Calculate the Flow in [mm^3/s]
pump_flow = puflo.get_flow(gcode_necessary, ROBOT_VEL_CP)
for gcd, flo in zip(gcode_necessary, pump_flow):
    gcd["Flow"] = flo

# Calculate the value for the external axis (rpm or voltage)
pump_command = purpm.get_rpm(gcode_necessary, PUMP_CHARACTERISTIC_CURVE, ROBOT_VEL_CP)
...
if src is True:
    ...
    length_name = 25 if OUTPUT_SPLIT_SRC == False else 21
    ...
    krl_lines = mdfkr.krl_format(
        gcode_necessary[
            1:-1
        ],  # skip start and end position as they are part of start and end code
        ...
        split_layers=OUTPUT_SPLIT_SRC,
        project_name=filename_formatted,
    )
    if not OUTPUT_SPLIT_SRC:
        ...
        expkr.split_and_export_to_src(
            krl_lines=krl_lines,
            krl_name=filename_formatted,
    ...
    if not reachable:
        print("[ERROR] .src file not generated due invalid/unreachable point")
    if not location:
        print(
            "[ERROR] .src file not generated due to invalid location or scaling of object on printbed"
        )
...
repgc.plot_gcode(plotter, gcode_necessary[1:-1], "all", RHINO_LINE_TYPES_COLOR)

# Plot of the characteristic curve for the pump
repcc.plot_pump_curve(PUMP_CHARACTERISTIC_CURVE)
...
extended_gcode = rhext.add_point_info(gcode_necessary)

# Evaluate sub_sub Layers for the Layer "toolpath" inside the rhino file
sublayers = rhevl.evaluate_sublayers_printbed(extended_gcode)


# Get filepath of generated Rhino file
filepath = crtrh.initialize_rhino_file(
    OUTPUT_DIRECTORY_RHINO, OUTPUT_FILE_RHINO, layer_max, sublayers
)
...
if robot_file.exists() and robot_file.suffix.lower() == ".3dm":
    ...
    rhbot.import_step_file_to_rhino_file(
        ROBOT_3DM_FILE, robotroot_pos, filepath, "robot"
    )
else:
    print(
...
toolpath = drgrh.create_geometry(
