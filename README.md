...
| `printer.def.json`  | Base Input data for Cura (considers only the deepest nested arguments)             | <br/>To overwrite arguments quickly use Cura.cura_arguments in `setup.json` |
| `extruder.def.json` | Base Input data on extruder for Cura (considers only the deepest nested arguments) |                                                                             |
